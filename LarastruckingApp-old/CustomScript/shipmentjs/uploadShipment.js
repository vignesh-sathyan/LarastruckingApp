        ...
        if (glbShipmentData.length > 0) {
            ...
            for (var i = 0; i < glbShipmentData.length; i++) {
                ...
                if (glbShipmentData[i].IsDateExpired) {
                    ...
                    tableBody += "<td style='color:red'>" + ConvertDate(glbShipmentData[i].Date, true) + "</td>";
                }
                else {
                    tableBody += "<td>" + ConvertDate(glbShipmentData[i].Date, true) + "</td>";
                }
                ...
                if (glbShipmentData[i].DeliveryDate != null) {
                    ...
                    if (glbShipmentData[i].IsDeliveryDateExpired) {
                        tableBody += "<td style='color:red'>" + ConvertDate(glbShipmentData[i].DeliveryDate, true) + "</td>";
                    }
                    else {
                        tableBody += "<td>" + ConvertDate(glbShipmentData[i].DeliveryDate, true) + "</td>";
...
//#endregion//#region convert datetime stampfunction convertToJSONDate(strDate) {
...
//#endregion//#region bind pricing method , Pcs Type and Freight type dropdownfunction BindPricingMethodNFreightTypeNPcsType() {
...
}//#endregion//#region update record$("#btnUpdate").click(function () {    debugger;    var isValid = true;    var message = "";    var index = $("#rowValue").val();
    var rowdetail = glbShipmentData[index];
...
})//#endregion//#region $("#btnClear").click(function () {    $.confirm({
        title: 'Confirmation!',
    ...
    })    debugger});//#endregion//#region save grid data in excel$("#btnSaveData").click(function () {
    ...
    if (isvalid) {
        ...
        for (var i = 0; i < glbShipmentData.length; i++) {
            debugger
            glbShipmentData[i].Date = ConvertDate(glbShipmentData[i].Date, true);
            glbShipmentData[i].DeliveryDate = ConvertDate(glbShipmentData[i].DeliveryDate, true);
        }
...
});//#endregion//#region check file namefunction CheckFileName(_this) {    debugger;
