...
(function (global, factory) {
	...
	*/
	...
	var closestToOffset = function closestToOffset(elements, offset) {
	  ...
	  for (var i = 0; i < elements.length; i++) {
	    var el = elements[i],$el = $__default['default'](el);
	...
	MaskedInput = /*#__PURE__*/function () {
	  ...
	  function MaskedInput(options) {var _this = this;_classCallCheck(this, MaskedInput);
	    /** @private */
	    var o = this.o = $__default['default'].extend({}, MaskedInput.defaults, options);

	    var patterns = {};
	    MaskedInput.patternAddons.forEach(function (addon) {
	      patterns = $__default['default'].extend(patterns, addon);
	    });
	    patterns = $__default['default'].extend(patterns, o.patterns);
	    o.patterns = patterns;
	    ...
	    /** @public */
	    var $el = this.$el = $__default['default']('<div>').addClass(o.className || 'masked-input');
	    ...
	    p.$inputBackBuffer = $__default['default']('<span aria-hidden="true" style="position:absolute;z-index:-1;left:0;top:-9999px;white-space:pre;"/>');

	    // Hook up click event
	    $el.on('click', function (event) {
	      if (event.target !== event.currentTarget &&
	      $__default['default'](event.target).is(FOCUSABLE_SELECTOR)) return;

	      var offset = $__default['default'](event.currentTarget).offset();
	   ...
	   */_createClass(MaskedInput, [{ key: "_parseFormat", value: function _parseFormat(
	    ...
	    part, input) {var _this4 = this;
	      ...
	      if (isNewInput) {
	        $input = $__default['default']('<input>').data('part', part).prop('disabled', !p.enabled);
	        input = $input[0];
	      } else {
	    ...
	    part) {
	      return $__default['default']('<span style="white-space: pre">').text(part.text);
	    }
	    ...
	    input, alwaysConsiderPlaceholder, fallbackText) {
	      ...
	      var $input = $__default['default'](input),$backBuffer = p.$inputBackBuffer;
	    ...
	    event, input, part) {
	      ...
	      if (this._shouldMoveToNextFieldAfterInput(getSelectionRange(input), input.value, part)) {
	        $__default['default'](input).nextAll(TABBABLE_SELECTOR).first().focus();
	      }
	    ...
	    event, input, part) {
	      ...
	      if (keycode === KEY_ARROW_LEFT || keycode === KEY_ARROW_RIGHT) {
	        var isRtl = $__default['default'](input).css('direction') === 'rtl';

	        if (!isRtl && keycode === KEY_ARROW_LEFT || isRtl && keycode === KEY_ARROW_RIGHT) {
	          if (getSelectionRange(input).begin === 0) {
	            $__default['default'](input).prevAll(TABBABLE_SELECTOR).first().focus();
	          }
	        } else {
	    ...
	    index) {
	      ...
	      return $__default['default'].isArray(input) ? input.slice(0) : input;
	    }
	    ...
	    name, newValue) {
	      ...
	      if (arguments.length === 2) {
	        if (name === 'patterns') {
	          ...
	          MaskedInput.patternAddons.forEach(function (addon) {
	            o[name] = $__default['default'].extend(o[name], addon);
	          });

	          o[name] = $__default['default'].extend(o[name], newValue);
	        } else {
	          o[name] = newValue;
	        }
	      } else {
	      ...
	      if (inputEls.length > 1) {
	        if (arguments.length === 3 || _typeof(name) === 'object') {
	          ...
	          inputEls.forEach(function (el) {
	            that._fieldOption($__default['default'](el).data('part'), name, value);
	          });
	          ...
	          return inputEls.map(function (el) {return that._fieldOption($__default['default'](el).data('part'), name);});
	        }
	      } else {
	        if (arguments.length === 3) {

	          // Set the option/options for input
	          this._fieldOption($__default['default'](inputEls).data('part'), name, value);
	          ...
	          // Return value/values
	          return this._fieldOption($__default['default'](inputEls).data('part'), name);
	        }
	      }
	    }
