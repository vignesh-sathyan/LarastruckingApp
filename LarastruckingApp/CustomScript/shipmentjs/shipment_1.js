    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (let i = 0; i < response.ShipmentRoutesStop.length; i++) {
                ...
                tempval = response;
                values.pickupLocation = response.ShipmentRoutesStop[i].PickupLocation;
                values.DeliveryLocation = response.ShipmentRoutesStop[i].DeliveryLocation;
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response != null) {
                    ...
                    console.log("AiwayBill text: ", response.AirWayBill);
                    ...
                    for (var i = 0; i < response.ShipmentRoutesStop.length; i++) {
                        //console.log("response.ShipmentRoutesStop.length: ", response.ShipmentRoutesStop.length);
                        var objRouteStops = {};
...
function addRouteStop(isAppointmentRequireds) {
    ...
    var website = $(".ddlDeliveryLocation").attr("data-website");
    var phone = $(".ddlDeliveryLocation").attr("data-phone");
    var AirWayBill = $.trim($("#txtAirWayBill").val());
...
function GetJsonValue() {
    ...
    for (var i = 0; i < glbRouteStops.length; i++) {
        console.log("AirwayBill ShipRoute: ", glbRouteStops[i].AirWayBill);
...
function SendMessage() {
    ...
    //}
    //else if (values.CustomerPO != ""){
    //    AWB += values.CustomerPO;
    //}
    //else if (values.OrderNo != ""){
    //    AWB += values.OrderNo;
    //}
    //else if (values.CustomerRef != ""){
    //    AWB += values.CustomerRef;
    //}
    //else {
   ...
   // console.log("AWB: "+AWB);
    ...
    var driverid = values.ShipmentEquipmentNdriver[0].DriverId;
    var driverphone;
    ...
    if (values.AirWayBill != "") {
        commonStr += "NUM # " + values.AirWayBill + "\n";
    }
...
var btnSave = function () {
    ...
    $("#btnSave").on("click", function () {
        ...
        if (ShipmentEquipmentNdriverCheck != false) {
            var DriverName = values.ShipmentEquipmentNdriver[0].DriverName;
        }
        var PickupLocation = values.ShipmentRoutesStop[0].PickupLocation;
                ...
                if (values.ShipmentId > 0) {
                    ...
                    $.ajax({
                        ...
                        beforeSend: function () {
                            ...
                            console.log("shipment status: " + ddlstatus);
                            // const  = values.ShipmentEquipmentNdriver[0];
                            if (ShipmentEquipmentNdriver != false && prevalues.EquipmentName != "null" && ShipmentEquipmentNdriver.DriverName != "" && ddlstatus == "DISPATCHED") {
                                if (PickupLocation != prevalues.pickupLocation || DeliveryLocation != prevalues.DeliveryLocation || pallets != prevalues.Pallets || boxes != prevalues.Boxes || AWB != prevalues.AirWayBill || DriverName != prevalues.DriverName) {
                                    SendMessage();
                                }
                            }
