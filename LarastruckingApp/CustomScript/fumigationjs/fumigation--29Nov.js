        ...
        success: function (response) {
            for (let i = 0; i < response.GetFumigationRouteDetail.length; i++) {
                ...
                values.push({
                    ...
                    "AirWayBill": AWB,
                    "EquipmentName": response.FumigationEquipmentNDriver[i].EquipmentName
                });
            }
        }
    });
...
function SendMessage() {
    ...
    for (let i = 0; i < values.FumigationRouteDetail.length; i++) {
        ...
        driverid = values.FumigationRouteDetail[i].PickUpEquipmentNdriver[0].DriverId;
        var driverphone;
    }
        ...
        if (fumedit == true) {
            if (driverphone != "" || driverphone != "undefined") {
                ...
                $.ajax({
                    ...
                    data: "{ 'pickupDetails': '" + finalSMSBodyStr + "','phone': '" + driverphone + "'}",
                    contentType: 'application/json; charset=utf-8',
                    dataType: 'json',
                    success: function (msg) {
                        // Do something interesting here.
                    ...
                    if (glbRouteStops.length > 0) {
                        if (values.FumigationId > 0) {
                            ...
                            $.ajax({
                                ...
                                beforeSend: function () {
                                    showLoader();
                                    console.log("pickup: ", values.FumigationRouteDetail[0].PickUpEquipmentNdriver[0].DriverId);
                                    console.log("deliver: ", values.FumigationRouteDetail[0].DeliveryEquipmentNdriver[0].DriverId);
                                    if (values.FumigationRouteDetail[0].PickUpEquipmentNdriver[0].DriverId != "" && values.FumigationRouteDetail[0].DeliveryEquipmentNdriver[0].DriverId == 0) {
                                        SendMessage();
                                    }
                                    else if (values.FumigationRouteDetail[0].DeliveryEquipmentNdriver[0].DriverId != "") {
                                        SendDeliveryMessage();
                                    }
                                    else {
                                        alert("Equipment or driver is not assigned");
                                    }
