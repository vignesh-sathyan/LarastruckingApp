...
function RecalculateHourAndprice(_this) {
    ...
    //console.log("txtOutDate: " + txtOutDate + " : " + ccOutDateTime);
...
function BindTimeCardList(_data) {
    ...
    for (let i = 0; i <= 6; i++) {
        ...
        if (timeCardList.length > 0) {
            ...
            for (var j = 0; j < timeCardList.length; j++) {
                ...
                } else*/
                {
                    timeCard += '<tr><td><input type="text" style="display:none"  id="hdnIsRemovalFlag" value="false"/><input type="hidden"  id="hdnTimeCardId" value="' + timeCardList[j].Id + '"/><input type="hidden" id="hdndate" value="' + date + '"/><span id="spnDay">' + weekday[timeCardDate.getDay()] + '</span></td><td class="text-center"><div class="timeWrapper d-inline-flex align-items-center">' +
                        ...
                        '<input type="text" ' + rquired + ' onkeypress="return onlyNumeric(event)" id="txtOutMin" placeholder="MM" class="form-control mm"  ' + outOnfocusout + ' maxlength="2" value="' + outMin + '" /></div></td><td width="50px" id="add-icon">' + addIcon + removeIcon +
                        '</td><td>' + diffHrs + ':' + diffMins + '</td></tr>';
                    ...
                    timCardPrint += '<tr style="background:#fff;">' +
                        ...
                        '<td style="padding:7px;text-align:center;">' +
                        '<label style="background:#fff; height:35px; padding:5px 10px; font-size:17px; font-weight:400; line-height:1.5; color:#495057; border:0px solid #ced4da; border-radius:5px;">' + dateFormat(outDateyear, "mm-dd-yyyy") + '</label>' +
                        '<span>&nbsp;</span>' +
            ...
            timCardPrint += '<tr style="background:#fff;">' +
                '<td style="text-align:left; padding-left:10px; background:#f2f2f2;">' +
                '<span style="font-size:16px;">' + weekday[timeCardDate.getDay()] + '</span>' +
                '</td>' +
                '<td style="padding:7px;text-align:center; background:#f2f2f2;">' +
                ...
                '</td>' +
                '<td style="padding:7px;font-size:16px; background:#f2f2f2;">To</td>' +
                '<td style="padding:7px;text-align:center; background:#f2f2f2;">' +
                ...
                '<label style="background:#fff; height:35px; padding:5px 10px; font-size:16px; font-weight:400; line-height:1.5; color:#495057; border:0px solid #ced4da; border-radius:5px;">MM</label>' +
                '</td>' +
                '<td style="font-size:16px; background:#f2f2f2;">00:00</td>' +
                '</tr> ';
        }
    }
    if (totalSec > 0 || totalHour > 0 || totalMin > 0) {
...
function btnSendEmail() {
    ...
    //var printContents = document.getElementById("theimage9").innerHTML;
    var to = $("#txtTo").val();
