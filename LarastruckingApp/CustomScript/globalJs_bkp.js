...
function ConvertDateNew(jsondate, time = false) {
    ...
    if (jsondate != null && jsondate != "") {
       ...
       // console.log(timeValues); // 👉️ "07:30:14"
        ...
        const parsedDates = new Date(+year, +month - 1, +day, +hours, +minutes, +seconds);


        var parsedDate = new Date(parseInt(jsondate.substr(6)));
        //var parsedDates = new Date(parseInt(date.substr(6)));
       // console.log("parsedDate: " + parsedDates);
        var dt = parsedDates.getDate() < 10 ? ('0' + parsedDates.getDate()) : parsedDates.getDate();
        var mnt = parsedDates.getMonth() < 9 ? ('0' + (parsedDates.getMonth() + 1)) : (parsedDates.getMonth() + 1).toString();
        var yr = parsedDates.getFullYear().toString();

        var sec = parsedDates.getSeconds();
        sec = sec < 10 ? '0' + sec : sec;

        var min = parsedDates.getMinutes();
        min = min < 10 ? '0' + min : min;

        var hr = parsedDates.getHours();
        hr = hr < 10 ? '0' + hr : hr;
