--exec usp_GetShipment_OrderTaken_List'','ShipmentId','asc','',1000          
          
CREATE PROC [dbo].[usp_GetShipment_OrderTaken_List]                      
...
select  [TotalCount]= COUNT(TotalCountRow) over(), ShipmentId,AirWayBill,CustomerPO,StatusId, StatusName,CustomerID,CustomerName,PickupDate,DeliveryDate,PickupLocation,DeliveryLocation,Quantity,CreatedByName,IsReady              
       from view_GetOrderTakenShipment where              
    ...
    (                      
      ...
      OR CreatedByName LIKE '%' + @SearchTerm + '%'                         
                        )                 
      group by TotalCountRow,   ShipmentId,AirWayBill,CustomerPO,StatusId, StatusName,CustomerID,CustomerName,PickupDate,DeliveryDate,PickupLocation,DeliveryLocation,Quantity,CreatedByName,IsReady                
ORDER BY CASE                      
        ...
        CASE                      
            ...
            END ASC,                      
                      CASE                      
                          WHEN(@SortColumn = 'DeliveryLocation'                      
                               AND @SortOrder = 'desc')                      
                          THEN DeliveryLocation                                    END DESC,      
         CASE                      
                          WHEN(@SortColumn = 'CreatedByName'                      
                               AND @SortOrder = 'asc')                      
                          THEN CreatedByName                      
            END ASC,                      
                      CASE                      
