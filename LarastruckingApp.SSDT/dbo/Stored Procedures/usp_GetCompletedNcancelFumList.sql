CREATE PROC [dbo].[usp_GetCompletedNcancelFumList]                      
(    
 ...
 AS (SELECT DISTINCT                      
   ...
   -- GROUP BY FED.EquipmentId,                       
          ...
          FOR XML PATH('')                      
             ...
             CTE_DeliveryEquipment(FumigationIds,DeliveryEquipment)                      
             AS (SELECT DISTINCT                      
                        (FumigationId),                       
                        STUFF(                      
                 (                      
 ...
 SELECT '|' + CONVERT(VARCHAR(MAX), FRS.VendorNConsignee)                      
  ...
  (                      
     ...
     FUM.StatusId
            ...
            -- INTO #FUMIGATIONDATA                      
             FROM tblFumigation FUM                      
                  Left JOIN [dbo].[tblCustomerRegistration] CR ON FUM.CustomerId = CR.CustomerId                      
             ...
             WHERE FUM.IsDeleted = 0  AND (SS.StatusId=11 OR SS.StatusId=8)    
...
OR CustomerPO LIKE '%' + @SearchTerm + '%'                      
      ...
      where (                
      ((NullIf(@StartDate, '') IS NULL)  OR CAST(PickUpArrival AT TIME ZONE 'UTC' AT TIME ZONE 'Eastern Standard Time' AS DATE) >= @StartDate )                  
   AND ((NullIf(@EndDate, '') IS NULL)  OR CAST(PickUpArrival AT TIME ZONE 'UTC' AT TIME ZONE 'Eastern Standard Time' AS DATE) <=  @EndDate)                  
  )                 
   AND          
  (                
     ...
     AND(                
      ((NullIf(@DriverName, '') IS NULL)  OR PickUpDriver  LIKE '%' + @DriverName + '%' OR DeliveryDriver  LIKE '%' + @DriverName + '%')                  
   )                     
          ORDER BY CASE                      
    ...
    END DESC,                      
             ...
             (                      
                 SELECT PickUpLocation                      
                 FROM [dbo].[GetFumigationLocation](FumigationId)                      
             ), '')                      
                      END ASC,                      
                      CASE                      
                          WHEN(@SortColumn = 'PickUpLocation'                      
