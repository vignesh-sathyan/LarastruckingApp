CREATE FUNCTION [dbo].[fun_GetShipmentQuantity]  
     (  
     @ShipmentId int  
     )  
     returns table as   
   ...
   (select CASE WHEN SUM(T1.NoOfBox)>0 THEN REPLACE(CAST(SUM(T1.NoOfBox) AS nvarchar),'.00','')+' BOX, ' ELSE '' END from tblShipmentFreightDetail T1 WHERE T1.ShipmentId=TSFD.ShipmentId AND T1.IsDeleted=0) AS BOX,  
    (select CASE WHEN SUM(T1.Weight)>0 THEN REPLACE(CAST(SUM(T1.Weight) AS nvarchar),'.00','')+' LBS, ' ELSE '' END  from tblShipmentFreightDetail T1 WHERE T1.ShipmentId=TSFD.ShipmentId AND (T1.Unit='LBS' OR T1.Unit='LB') AND T1.IsDeleted=0) AS LB,  
