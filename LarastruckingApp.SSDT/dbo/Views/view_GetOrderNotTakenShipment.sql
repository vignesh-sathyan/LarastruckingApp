...
CREATE view [dbo].[view_GetOrderNotTakenShipment]                        
As                            
...
CTE_PickupNDeliveryLocation(ShipmentId,PickupDate,DeliveryDate,PickupLocation,DeliveryLocation)                                
...
STRING_AGG (CONVERT(VARCHAR(MAX),CAST (DeliveryDateTime AT TIME ZONE 'UTC' AT TIME ZONE 'Eastern Standard Time' AS varchar )), '|')   As DeliveryDateTime,STRING_AGG ( CONCAT(AD.CompanyName,'||',AD.Address1,' ',AD.City,' ',STT.Name,' ',Ad.Zip              
      ...
      As DeliveryLocation  
   -- ,STRING_AGG ( ISNULL(SFD.Temperature,0),'$') AS Temperature,   STRING_AGG (ISNULL(SFD.Commodity,''),'$') AS Commodity    
FROM tblShipment SHP With(NOLOCK)                      
Left join  tblShipmentRoutesStop TSRS on TSRS.ShippingId = SHP.ShipmentId     
--Left Join tblShipmentFreightDetail SFD ON TSRS.ShippingRoutesId=SFD.ShipmentRouteStopeId                       
...
CTE_TempCommodity(ShipmentId,Temperature,Commodity)                                
AS( SELECT Distinct (SHP.ShipmentId), STRING_AGG ( ISNULL(SFD.Temperature,0),'$') AS Temperature,   STRING_AGG (ISNULL(SFD.Commodity,''),'$') AS Commodity    
 FROM tblShipment SHP With(NOLOCK)                      
Left join  tblShipmentRoutesStop TSRS on TSRS.ShippingId = SHP.ShipmentId     
Left Join tblShipmentFreightDetail SFD ON TSRS.ShippingRoutesId=SFD.ShipmentRouteStopeId         
 WHERE  SHP.IsDeleted=0 AND TSRS.IsDeleted=0 and SHP.StatusId not in (1,8, 11)                               
...
,QT.Quantity, ISNULL( (select TOP(1)   (case when SHP.StatusId=SSH.StatusId then 1 else 0 end) ApproveStatus from tblShipmentStatusHistory SSH where SSH.ShipmentId=SHP.ShipmentId order by ShipmentStatusHistoryId desc),0) as ApproveStatus ,TMPCOMM.Temperature,  
TMPCOMM.Commodity     
...
Left JOIN CTE_TempCommodity TMPCOMM ON SHP.ShipmentId=TMPCOMM.ShipmentId                                
